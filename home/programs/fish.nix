...

{
  ...
  home.shellAliases = {
    # Pass SSH_AUTH_SOCK to sudo so root can access your SSH keys
    nrs = "sudo --preserve-env=SSH_AUTH_SOCK nixos-rebuild switch --flake ~/my-home-manager#main";
    nrs-work = "sudo --preserve-env=SSH_AUTH_SOCK nixos-rebuild switch --flake ~/my-home-manager#work";
    ...
    toggle-audio-out = "pactl set-sink-port 0 $(pactl list sinks | awk '/Active Port:/{print $3}' | grep -q 'analog-output-headphones' && echo analog-output-lineout || echo analog-output-headphones)";
     ...
     glock="./gradlew deleteLock generateLock saveLock clean build";
      gadd-nolock="find . -type f ! -nabraveme '*.lock' -print0 | xargs -0 git add";
