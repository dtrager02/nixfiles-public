...

{
  ...
  systemd = {
    ...
    # Out-of-memory monitoring and remediation
    oomd.enable = true;
  };
  ...
  # Compressed RAM swap for better OOM resilience
  zramSwap.enable = true;
  zramSwap.writebackDevice = "/dev/nvme0n1p2";
  ...
  systemd.services.disable-auto-mute = {
    ...
    serviceConfig = {
      Type = "oneshot";
      ExecStart = "${pkgs.bash}/bin/bash -c 'for card in 1 2 3 4; do ${pkgs.alsa-utils}/bin/amixer -c $card sset \"Auto-Mute Mode\" Disabled || true; done'";
